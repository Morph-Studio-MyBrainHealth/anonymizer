...
def determine_pii_type_from_content(key, value):
    ...
    if any(term in key_lower for term in ['aiadl', 'smmse', 'iqcode', 'ace_iii', 'ace-iii', 'moca', 'mmse',
                                          'memory', 'fluency', 'language', 'visuospatial', 'attention', 
                                          'orientation', 'total', 'score', 'domain_performance']):
        return 'COGNITIVE_SCORE'
    
    # Check if value looks like a score (numeric, fraction, or decimal)
    elif isinstance(value, (int, float)) or (isinstance(value, str) and re.match(r'^\d+(/\d+)?(\.\d+)?
...
def lambda_handler(event, context):
    ...
    return response,  # Decimal scores like "4.42" or whole numbers like "35"
        ]
...
def determine_pii_type_from_key(key):
    """
    Determine the PII type based on the key name.
    Enhanced with more specific mappings including cognitive assessments.
    """
    key_lower = key.lower()
    
    # Check for cognitive assessment scores first
    if any(x in key_lower for x in ['aiadl', 'smmse', 'iqcode', 'ace_iii', 'ace-iii', 'moca', 'mmse',
                                    'memory', 'fluency', 'language', 'visuospatial', 'attention', 
                                    'orientation', 'total', 'score', 'domain_performance']):
        return 'COGNITIVE_SCORE'
    # Check for specific patterns
...
def should_anonymize_key(key):
    """
    Determine if a key likely contains PII/PHI based on its name.
    Enhanced to catch more medical fields including cognitive assessments.
    """
    pii_keywords = [
        'name', 'clinic_name', 'provider', 'doctor', 'physician',
        'date', 'dob', 'birth', 'address', 'phone', 'email',
    ...
    key_lower = key.lower()
    # Also check for exact matches for assessment abbreviations
    assessment_abbreviations = ['aiadl', 'smmse', 'iqcode', 'ace_iii', 'ace-iii', 'moca', 'mmse']
    if key_lower in assessment_abbreviations:
        return True
